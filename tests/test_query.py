...
from tests.test_app.models import AnotherMockModel, MockModel
...
class ElasticsearchSearchQueryTestCase(TestCase):
    ...
    def test_narrow_sq(self):
        sqs = SearchQuerySet(using='default').narrow(SQ(foo='moof'))
    ...
    def test_query__in(self):
        sqs = SearchQuerySet(using='default').filter(id__in=[1, 2, 3])
